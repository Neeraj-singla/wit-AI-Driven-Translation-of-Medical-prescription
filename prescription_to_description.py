...
def prescription_translate_description(text):
    ...
    input_copy = [[input]]
    predict_df = pd.DataFrame.from_records(input_copy , columns=['P_SIG'])
    predict_df['P_SIG'] = [entry.lower() for entry in predict_df['P_SIG']]
    predict_df['P_SIG']= [word_tokenize(entry) for entry in predict_df['P_SIG']]
    predict_df["P_SIG"]= predict_df["P_SIG"].astype(str)
    temp_slot = Tfidf_vect.transform(predict_df["P_SIG"])
    ...
    for i in range(len(predict_df)):
        ...
        if len(str_temp)==0:
            predicted.append(predict_df["P_SIG"][i])
            continue
        str1=first_slot[i]+" "
        tempo=[]
        try:
